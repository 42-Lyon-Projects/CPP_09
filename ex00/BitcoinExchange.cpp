...
bool BitcoinDate::date_has_good_content(const std::string &str) const {
    ...
    int arrayLength = (sizeof(littleMonth) / sizeof(littleMonth[0])) - 1;
...
void BitcoinExchange::load_data_from_strings(const std::string *str, size_t lineAmount, char delimiter,
                                             std::map<BitcoinDate, float> *map) throw (std::out_of_range,
    std::runtime_error) {
    for (size_t i = 1; i < lineAmount; ++i) {
        ...
        if (tokenPos == std::string::npos) {
            std::cout << "Error => Unable to load the line " << line << " (" << i + 1 << ")" << std::endl;
            std::cout << "  This data will not be loaded because the delimiter has not find !" << std::endl;
            continue;
        }
        ...
        try {
            ...
            if (errno == EINVAL || errno == ERANGE || *end_ptr != '\0') {
                errno = 0;
                throw std::runtime_error(
                    std::string("The string '").append(value).append("'").append(" can't be converted to number."));
            }
            if (delimiter == '|' && (btc_value > 1000 || btc_value < 0))
                throw std::runtime_error(std::string("Bad input in BTC value: ").append(value));
            std::pair<BitcoinDate, float> pair = std::pair<BitcoinDate, float>(btc_date, btc_value);
            map->insert(pair);
        } catch (BitcoinDate::InvalidDateException &ignored) {
            std::cout << "Error => Bad input.\n  The date '" << date << "' is invalid." << std::endl;;
        }
...
/**
 *
 * @param date a bitcoin date object
 * @return A pair with a timestamp key, and btc value at this timestamp
 */
std::pair<time_t, float> BitcoinExchange::find_nearest_pair(const BitcoinDate &date) const {
    ...
    for (std::map<BitcoinDate, float>::const_iterator it = bitcoins_values.begin(); it != bitcoins_values.end(); ++it) {
        const time_t timestamp = it->first.to_timestamp();
...
std::string *FileUtils::read_string_array(const std::string &path, size_t *amount) throw (std::runtime_error) {
    ...
    if (!readFileFlux.is_open()) throw std::runtime_error(std::string("Unable to open file ").append(path));
